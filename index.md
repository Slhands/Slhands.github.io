...
## 1.요구사항
- 시작화면,게임화면,엔딩화면 총 3개가있다.
- 시작화면에서 시작하기(GAME START) 클릭시 게임화면으로 이동
- 시작화면에는 시작하기(GAME START,나가기,옵션 버튼이 존재한다.
- 조작은 w s a d + 마우스를 이용한다
- 게임화면에는 현재상태(선생/학생)을 표시하는 마크1개,인벤토리아이콘1개,플레이어의 손,카메라뷰로 표시된다.
- 게임오버의 조건은 "에너미와 접촉시/에너미의 이벤트트리거 작동시" 이다.
## 세부사항
[1주차(3주)]
